...
import os
import numpy as np
from src.database import create_and_populate_database, delete_data_from_database
from src.secure_computation import generate_paillier_keypair, secure_distance_computation, decrypt_and_unmask_distances

def delete_entry(label, db_name="data/fingerprints.db"):
    """Delete an entry from the database based on the label."""
    delete_data_from_database(db_name, label)
